...
func GetGameByID(id int64) (game Game, err error) {
	counter := 0
	rows, err := database.Database.Query("SELECT g.id AS id, g.name AS name, g.created AS created, a.id AS teamA_id, a.name AS teamA_name, a.color AS teamA_color, a.color_code AS teamA_color_code, a.logo AS teamA_logo, a.created AS teamA_created, b.id AS teamB_id, b.name AS teamB_name, b.color AS teamB_color, b.color_code AS teamB_color_code, b.logo AS teamB_logo, b.created AS teamB_created FROM game AS g INNER JOIN team AS a ON  g.teamA = a.id INNER JOIN team AS b ON  g.teamB = b.id WHERE g.id=?", id)
	...
	for rows.Next() {
		err = rows.Scan(&game.ID, &game.Name, &game.Created, &game.TeamA.ID, &game.TeamA.Name, &game.TeamA.Color, &game.TeamA.ColorCode, &game.TeamA.Logo, &game.TeamA.Created, &game.TeamB.ID, &game.TeamB.Name, &game.TeamB.Color, &game.TeamB.ColorCode, &game.TeamB.Logo, &game.TeamB.Created)
		if err != nil {
			return
		}
		counter++
...
func GetSimpleGameByID(id int64) (game GameInput, err error) {
	counter := 0
	rows, err := database.Database.Query("SELECT id, name, teamA, teamB, created FROM game WHERE id=?", id)
	...
	for rows.Next() {
		err = rows.Scan(&game.ID, &game.Name, &game.TeamA, &game.TeamB, &game.Created)
		if err != nil {
			return
		}
		counter++
